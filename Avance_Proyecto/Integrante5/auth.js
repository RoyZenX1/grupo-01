...
function normalizarRol(rol) {
  ...
  const rolNormalizado = String(rol).trim().toLocaleLowerCase("es-PE");
...
function normalizarUsuario(usuario) {
  return {
    ...
    email: String(usuario.email).trim().toLocaleLowerCase("es-PE"),
...
function iniciarSesion(email, clave) {
  ...
  const emailLimpio = String(email).trim().toLocaleLowerCase("es-PE");
  ...
  if (emailLimpio === "" || claveLimpia === "") {
    throw new Error("Ingresa el correo y la clave.");
  }

  // Busca una cuenta cuyo correo y clave coincidan exactamente.
  const usuario = usuarios.find(
    (usuario) => usuario.email === emailLimpio && usuario.clave === claveLimpia,
  );

  // Si no hay coincidencia, el acceso se rechaza.
  if (!usuario) {
    throw new Error("Credenciales incorrectas. Verifica tu correo y clave.");
  }
...
function obtenerSesion() {
  try {
    ...
    const usuario = usuarios.find((usuario) => usuario.email === sesion?.email);
...
function rutaRaiz() {
  ...
  return /\/integrante/i.test(window.location.pathname) ? "../" : "";
}
